...
err_eta = reshape(out.err_eta, [3, length(tout)]);

nexttile
plot(tout(1:501), err_eta(:, 1:501))
ylabel("Orientation error [rad]")
xlabel("t [s]")
legend('e\_phi', 'e\_theta', 'e\_psi', 'Orientation', 'horizontal','Location','northoutside');

err_eta_dot = reshape(out.err_eta_dot, [3, length(tout)]);
...
nexttile
plot(tout(501:end), err_eta(:, 501:end))
ylabel("Orientation error [rad]")
xlabel("t [s]")
legend('e\_phi', 'e\_theta', 'e\_psi', 'Orientation', 'horizontal','Location','northoutside');
